...
// ─────────────────────────────────────────────────────────────────────────────
export const seedDatabase = async (programmatic = false) => {
  try {
    if (!programmatic) {
      await connectDB();
    }
    ...
    await Announcement.create([
      ...
      },
      {
        ...
        content:        "TechVision 2025 is scheduled on 28th–30th of this month. Register now — prizes worth ₹1,0,000!",
        category:       "event",
        author:         admin._id,
        targetAudience: ["all"],
        isPinned:       false,
...
if (!process.env.INTEGRATED_SERVER) {
  seedDatabase();
}
